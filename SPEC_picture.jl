...
# specout = readspec.ReadBoundary("./Paper2/data/G3V01L0Fi.002.sp.h5")

speceq = readspec.SPECEquilibrium("Paper2/data/G3V01L0Fi.002.sp.h5")
...
inset = 0.90
function source(X,t)
    s,_ = readspec.find_sθζ(X,0.0,speceq,1) # This is cubersome and should be replaced, maybe by a nearest neighbour interpolation
    s = (s + 1)/2 # s ∈ [0,1]
    tmp = exp(-s^2 / 0.4^2)
    if (inset - s) > 0
        return tmp
...
save("Paper2//data//SPEC_GridPoincare.png", f, px_per_unit=4)
