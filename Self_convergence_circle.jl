...
if rejection_form
    dirname = string("./data/SingleIslandSelf_rejection_$(order)_$(EXP)_$(dilation)_$(g0b)_$(α)")
else
    dirname = string("./data/SingleIslandSelf_$(order)_$(EXP)_$(dilation)_$(g0b)_$(α)")
end
@show dirname
...
if compute_reference
    ...
    if save_reference
        jldsave(string("./data/SISC_ref_EXP$(EXP)_gamma$(dilation)_g0b$(g0b)_alpha$(α)"); reference_u=reference_u)
    end
else
    nx = ny = reference_resolution
    reference_Dom = FaADE.Grid.squared_circle([(nx, ny) for _ in 1:5], radial_packing=packfn)
    @load string("./data/SISC_ref_EXP$(EXP)_gamma$(dilation)_g0b$(g0b)_alpha$(α)") reference_u
end
...
if save_rates
    ...
    CSV.write("./data/D_SISC_O$(order)_EXP$(EXP)_gamma$(dilation)_g0b$(g0b)_alpha$(α).csv", df)
end
